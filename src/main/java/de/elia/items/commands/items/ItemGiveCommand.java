...
public class ItemGiveCommand extends Command {
  ...
  public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
    ...
    if (sender instanceof Player player) {
          ...
          if (ComplexItem.SAVED.containsKey(complex)) {
            ...
            if (target != null) {
              if (args[1].equalsIgnoreCase("...")) {
                message(player, red("... Ist ein Beispiel für hörere Zahlen als 9 und hat somit keine Funktion."));
                return false;
              }
              Item.give(target, Complex.valueOf(args[0]), Integer.parseInt(args[1]));
              messageWithPrefix(player, gray("Du hast dem Spieler ").append(aqua(target.getName())).append(gray(" das item ")).append(aqua(args[0])).append(gray(" gegeben")));
              return true;
            }
            message(player, red(args[2] + "ist nicht online oder existiert nicht!"));
            return false;
          }
          return false;
        }
      ...
      messageWithPrefix(player, red("Du hast keine Rechte für diesen Command!"));
      return false;
    }
